...
function on_exit -p %self
    if not contains $argv[3] 0
        echo [(set_color --bold red) FAIL (set_color normal)] "dotfile setup failed!"
    end
end
...
function install_dotfiles
    ...
    for src in $DOTFILES_ROOT/fish/completions/*
        link_file $src $__fish_config_dir/completions/(basename $src) backup
        or abort "failed to link completion file $src"
    end
end
